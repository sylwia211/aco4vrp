...
For I1 = 1 To Vehicle
    ...
    For I2 = Path(I1 - 1).Ctrl + 1 To Path(I1).Ctrl - 2
        DeltaTao(Upshot(I2), Upshot(I2 + 1)) = DeltaTao(Upshot(I2), Upshot(I2 + 1)) + (BestRoute(0).Covered / ObjFunct) ^ p
        DeltaTao(Upshot(I2 + 1), Upshot(I2)) = DeltaTao(Upshot(I2 + 1), Upshot(I2)) + (BestRoute(0).Covered / ObjFunct) ^ p
    Next I2
Next I1
...
Dim Max() As Double
Dim Prom() As Double
ReDim Min(1 To GenerationNext)
ReDim Max(1 To GenerationNext)
ReDim Prom(1 To GenerationNext)
...
For I = 1 To GenerationNext
    Prom(I) = 0
    For J = 1 To nAnts
        If J = 1 Or History(J, I) < Min(I) Then
            Min(I) = History(J, I)
        End If
        If J = 1 Or History(J, I) > Max(I) Then
            Max(I) = History(J, I)
        End If
        Prom(I) = Prom(I) + History(J, I) / nAnts
        Form3.PSet (I, History(J, I)), vbRed
    Next J
    If I <> 1 Then
        Form3.Line (I - 1, Min(I - 1))-(I, Min(I)), vbGreen
        Form3.Line (I - 1, Max(I - 1))-(I, Max(I)), vbBlue
        Form3.Line (I - 1, Prom(I - 1))-(I, Prom(I)), vbWhite
    End If
Next I
...
If Int(Route(0).Covered * 100000) / 100000 < Int(RouteAnt(0).Covered * 100000) / 100000 Then
    NLS = NLS + 1
    Call AccDeltaTao(Solution(), Route(), Route(0).Covered, UBound(Route) - 1)
...
Do
    ...
    Call AccDeltaTao(BestSolution(), BestRoute(), Route(0).Covered, BestNv)
...
If Release = True Then
    ...
    Call AccDeltaTao(BestAnt(), RouteAnt(), Route(0).Covered, UBound(RouteAnt) - 1)
