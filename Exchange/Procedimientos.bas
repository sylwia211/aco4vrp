...
Sub Abstract(K As Integer)

Summary(NProblem, 1, K + 1) = BestRoute(0).Covered
Summary(NProblem, 2, K + 1) = BestNv
Summary(NProblem, 3, K + 1) = Timer - Time
...
For I1 = 1 To Vehicle
    ...
    For I2 = Path(I1 - 1).Ctrl + 1 To Path(I1).Ctrl - 1
        For I3 = I2 + 1 To Path(I1).Ctrl - 1
...
If Problem = "Todos" Then
    ...
    For NProblem = 1 To 14
        ...
        'Colectar resultados
        Summary(NProblem, 1, 6) = BestRoute(0).Covered
        Summary(NProblem, 2, 6) = BestNv
        Summary(NProblem, 3, 6) = Timer - Time
                ...
                For I2 = Route(I1 - 1).Ctrl + 1 To Route(I1).Ctrl - 1
                    For I4 = Route(I3 - 1).Ctrl + 1 To Route(I3).Ctrl - 1
                        If Route(I1).Cused + Dem(Solution(I4)) <= Capv Then
                            If (Route(I1).Tused + TimeS(Solution(I4)) + Dist(Solution(I2), Solution(I4)) + Dist(Solution(I4), Solution(I2 + 1)) - Dist(Solution(I2), Solution(I2 + 1))) <= TimeC Then
...
If Route(0).Covered < RouteAnt(0).Covered Then
    Call AccDeltaTao(Solution(), Route(), nAnts * 2 * Route(0).Covered, UBound(Route) - 1)
End If
...
For I1 = 1 To 14
    Print #3, I1 & vbTab & Int(Summary(I1, 1, 6) * 100) / 100 & vbTab & Summary(I1, 2, 6) & vbTab & Int(Summary(I1, 3, 6) * 1000) / 1000
Next I1
Close #3

For NProblem = 1 To 14
    Open App.Path & "\Resultados\Summary " & NProblem & ".txt" For Output As #3
    Print #3, "Costo" & vbTab & "Nv" & vbTab & "Tiempo"
    For I1 = 1 To 6
        Print #3, Int(Summary(NProblem, 1, I1) * 100) / 100 & vbTab & Summary(NProblem, 2, I1) & vbTab & (Int(Summary(NProblem, 3, I1) * 1000) / 1000)
    Next I1
    Close #3
Next NProblem
...
'    If Problem = "Todos" Then
        For K = 0 To 4
            If GenerationNext = 10 ^ K Then
                Call Abstract(K)
                'If K = 1 Then NProblem = 14
                Exit For
            End If
        Next K
'    End If
...
If Release = True Then
    Call AccDeltaTao(BestAnt(), RouteAnt(), nAnts * 2 * RouteAnt(0).Covered, UBound(RouteAnt) - 1)
...
For I = 1 To Nodes
    If Assigned(I) = False Then
        ...
        If Dem(I) + Route(Nv).Cused <= Capv Then
            ...
            If TimeC <> 0 And TimeS(I) + Route(Nv).Tused + Dist(I, Solution(Nearest(I, 2) - 1)) + Dist(I, Solution(Nearest(I, 2) + 1)) - Dist(Solution(Nearest(I, 2) - 1), Solution(Nearest(I, 2) + 1)) > TimeC Then
